 ...
 */
@Controller
public class XiZangDianXin_Controller extends AbstractDianXinController {

	private static final String TYPE = "XiZangDianXin_Controller";
	//ShanXiDianXin_Controller
	@RequestMapping(value = "/putong_XiZang_dianxin_vertifyLogin", method = RequestMethod.POST)
	...
	public Map<String, String> putong_vertifyLogin(boolean init, FormData fd) { 
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				final XiZangDianXin dx = new XiZangDianXin(spider, user, phone, fwpassword, authCode, util);
	...
	@RequestMapping(value="/putong_xizang_dianxin_GetAuth",method=RequestMethod.POST)
	...
	public Map<String, String> getAuth(boolean init, FormData fd) {
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				//3.创建业务流程处理类
				final XiZangDianXin dx = new XiZangDianXin(spider, util);
