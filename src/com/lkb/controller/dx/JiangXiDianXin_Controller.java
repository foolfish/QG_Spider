 ...
 */
@Controller
public class JiangXiDianXin_Controller extends AbstractDianXinController {
	private static final String TYPE = "JiangXiDianXin_Controller";
	...
	public Map<String, String> putong_vertifyLogin(boolean init, FormData fd) {
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				if(user!=null&&!user.getPhone().equals(user.getLoginName())){
					user.setPhone(user.getLoginName());
				}
				final JiangXiDianXin dx = new JiangXiDianXin(spider, user, phone, fwpassword, authCode, util);
