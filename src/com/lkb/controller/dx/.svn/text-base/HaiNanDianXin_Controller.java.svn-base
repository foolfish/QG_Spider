...
import com.lkb.thirdUtil.dx.HaiNanDianXin;
 ...
 */
@Controller
public class HaiNanDianXin_Controller extends AbstractDianXinController {
	
	private static final String TYPE = "HaiNanDianXin_Controller";
	
	@RequestMapping(value = "/putong_hainan_dianxin_vertifyLogin", method = RequestMethod.POST)
	...
	public Map<String, String> putong_vertifyLogin(boolean init, FormData fd) { 
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				//spider.getSite().setTimeOut(1000*20);
				final User user = getUser(currentUser, phone, Constant.DIANXIN);

				final HaiNanDianXin dx = new HaiNanDianXin(spider, user, phone, fwpassword, authCode, util);
	...
	@RequestMapping(value="/putong_hainan_dianxin_GetAuth",method=RequestMethod.POST)
	...
	public Map<String, String> getAuth(boolean init, FormData fd) {
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				Spider spider = SpiderManager.getInstance().createSpider(currentUser, TYPE);
				//3.创建业务流程处理类
				final HaiNanDianXin dx = new HaiNanDianXin(spider, util);
