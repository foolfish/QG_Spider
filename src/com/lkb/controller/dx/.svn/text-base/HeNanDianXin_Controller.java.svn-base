...
import com.lkb.thirdUtil.dx.HeNanDianXin;
import com.lkb.warning.WarningUtil;


@Controller
public class HeNanDianXin_Controller extends AbstractDianXinController {	
	private static final String TYPE = "HeNanDianXin_Controller";
	//fifth
	@RequestMapping(value = "/putong_henan_dx_reqService", method = RequestMethod.POST)
	...
	public Map<String, String> reqService(boolean init, FormData fd) { 
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				final HeNanDianXin dx = new HeNanDianXin(spider, user, phone, fwpassword, authCode, util);
				dx.requestAllService(duanxin);
	...
	//fourth
	@RequestMapping(value = "/putong_henan_dx_sms", method = RequestMethod.POST)
	...
	//second
	@RequestMapping(value = "/putong_henan_dx_vertifyLogin", method = RequestMethod.POST)
	...
	public Map<String, String> putong_vertifyLogin(boolean init, FormData fd) { 
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			...
			try {
				...
				final HeNanDianXin dx = new HeNanDianXin(spider, null, phone, fwpassword, authCode, util);
				dx.goLoginReq();
				SpiderManager.getInstance().startSpider(spider, null, dx);	
				dx.waitStatus();
				setResult(dx, map);			
	...
	//first
	@RequestMapping(value="/putong_henan_dx_GetAuth",method=RequestMethod.POST)
	...
	public Map<String, String> getAuth(boolean init, FormData fd) {
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				final HeNanDianXin dx = new HeNanDianXin(spider, util);
