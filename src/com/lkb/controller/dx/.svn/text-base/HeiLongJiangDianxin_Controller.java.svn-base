 ...
 */
@Controller
public class HeiLongJiangDianxin_Controller extends AbstractDianXinController {	
	private static final String TYPE = "HeiLongJiangDianxin_Controller";

	@RequestMapping(value = "/putong_hlj_dianxin_vertifyLogin", method = RequestMethod.POST)
	public @ResponseBody Map<String, String> putong_hlj_dianxin_vertifyLogin(HttpServletRequest req, HttpServletResponse resp, Model model) {
	...
	public Map<String, String> putong_vertifyLogin(boolean init, FormData fd) { 
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				final HeiLongJiangDianXin dx = new HeiLongJiangDianXin(spider, user, phone, fwpassword, authCode, util);
				dx.goLoginReq();
	...
	@RequestMapping(value="/putong_hlj_dianxin_GetAuth", method = RequestMethod.POST)
	...
	public Map<String, String> getAuth(boolean init, FormData fd) {
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				final HeiLongJiangDianXin dx = new HeiLongJiangDianXin(spider, util);
