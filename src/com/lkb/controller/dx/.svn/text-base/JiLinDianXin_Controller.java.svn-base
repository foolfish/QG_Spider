...
import com.lkb.thirdUtil.dx.JLDianxin;
 ...
 */
@Controller
public class JiLinDianXin_Controller extends AbstractDianXinController {
	private static final String TYPE = "JiLinDianXin_Controller";
	...
	public Map<String, String> putong_vertifyLogin(boolean init, FormData fd) {
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				final JLDianxin dx = new JLDianxin(spider, user, phone, fwpassword, authCode, util);
	...
	public Map<String, String> getAuth(boolean init, FormData fd) {
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				//3.创建业务流程处理类
				final JLDianxin dx = new JLDianxin(spider, util);
