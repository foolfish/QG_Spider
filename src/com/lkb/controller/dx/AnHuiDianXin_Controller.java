...
import com.lkb.thirdUtil.dx.AnHuiDianXin;
 ...
 */
@Controller
public class AnHuiDianXin_Controller extends AbstractDianXinController {

	private static final String TYPE = "AnHuiDianXin_Controller";

	@RequestMapping(value = "/putong_anhui_dianxin_vertifyLogin", method = RequestMethod.POST)
	...
	public Map<String, String> putong_vertifyLogin(boolean init, FormData fd) { 
		...
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				final AnHuiDianXin dx = new AnHuiDianXin(spider, user, phone, fwpassword, authCode, util);
	...
	@RequestMapping(value="/putong_anhui_dianxin_GetAuth",method=RequestMethod.POST)
	...
	public Map<String, String> getAuth(boolean init, FormData fd) {
		...
		util.setContext(warningService, currentUser);
		Map<String,String> map=new HashMap<String,String>();
		//1.做一些初始化 TYPE：类型值，保证唯一，比如AnHuiDianXin_Controller,简单可以直接用类名  userName：手机号
		if (!init || RobotUtil.init(fd.getRequest(), TYPE, phone)) {
			try {
				...
				//3.创建业务流程处理类
				final AnHuiDianXin dx = new AnHuiDianXin(spider, util);
