...
public class ThreadPool extends ThreadGroup {  
    ...
    private LinkedList<Task> workQueue;      //工作队列  
    private static String threadPoolID = "loginSpider";  //线程池的id  
    public static int MAX_POOL_SIZE = 300;//最大线程 ,暂没接入
    public static int MIN_POOL_SIZE = 50;//最小线程
    ...
    public ThreadPool() {  //poolSize 表示线程池中的工作线程的数量  
    	...
    	workQueue = new LinkedList<Task>();  //创建工作队列  
    ...
    private synchronized Task getTask(int threadid) throws InterruptedException {  
        while(workQueue.size() == 0) {  
            if(isClosed) return null;  
            System.out.println("工作线程"+threadid+"等待任务...");  
//            wait(5000);             //如果工作队列中没有任务,就等待任务  
