...
public class YidongUtils {
	public List<Map> getYDBill(String phone,IMobileTelService mobileTelService,IPhoneNumService phoneNumService,IMobileDetailService mobileDetailService,IUserService userService){
		Map map=new HashMap();
		map.put("teleno", phone);
		List<Map> list=mobileTelService.getMobileTelForReport1(map);
		if(list.get(0)!=null){
			String avg = "0";
			if(list.get(0).get("avg")!=null){
				avg=list.get(0).get("avg").toString();
				avg=avg.substring(0,avg.indexOf("."));
			}
			
	
			DateFormat df=new SimpleDateFormat("yyyy-MM-dd");
			String earlest=list.get(0).get("earlest").toString().substring(0,10);
			Date date=null;
			try {
			...
			userMap.put("usersource", Constant.YIDONG);
			...
			List<Map> list1=mobileDetailService.getMobileDetailForReport2(map);
			String lateststr = "";
		    if(list1.get(0)!=null&&list1.get(0).get("latest")!=null){
	...
	//报告获取联系人信息
		public List<Map> getLTContacts(String phone,IMobileDetailService mobileDetailService){
			Map<String,String> map=new HashMap();
			map.put("phone", phone);
			List<Map> list1=new ArrayList<Map>();
			List list=mobileDetailService.getMobileDetailForReport(map);
