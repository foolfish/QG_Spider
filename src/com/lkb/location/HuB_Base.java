...
import com.lkb.controller.dx.HuBDianXin_Controller;
import com.lkb.controller.lt.SHLianTong_Controller;
import com.lkb.controller.yd.HUBYIDONG_Controller;
/*湖北*/
public class HuB_Base extends AbstractBase {

	private static Logger logger = Logger.getLogger(HuB_Base.class);

	public void goWhere(String ptype, String currentUser, Model model) {
		if (ptype.contains("移动")) {
			model.addAttribute("putong_hub_yidong_url", "none");
		} else if (ptype.contains("联通")) {
			model.addAttribute("putong_sh_liantong_url", "none");
		} else if (ptype.contains("电信")) {
			model.addAttribute("putong_hub_dianxin_url", "none");
		} else {
	...
	public Result showValidateImg(PhoneNum phoneNum, FormData fd) {
		...
		if (isYiDong(phoneNum)) {
			HUBYIDONG_Controller controller = (HUBYIDONG_Controller )getBean(HUBYIDONG_Controller.class);
			Map<String,Object> map = controller.putongFirst(fd.getRequest(),controller.getLogin(fd));
			r.setForgetPassUrl(ConstantForgetPassword.hubyidong);
			setResultOld(r, map);
			...
			HuBDianXin_Controller controller = (HuBDianXin_Controller )getBean(HuBDianXin_Controller.class);
			Map<String,Object> map = controller.putongFirst(fd.getRequest(),controller.getLogin(fd));
			r.setForgetPassUrl(ConstantForgetPassword.hubdianxin);
			setResultOld(r, map);
		}
		return r;
	}
