 ...
 * 
 */
public class JS_Base extends AbstractBase {
	private static Logger logger = Logger.getLogger(JS_Base.class);
	public void goWhere(String ptype, String currentUser, Model model) {
		if (ptype.contains("联通")) {
			...
			model.addAttribute("putong_js_dianxin_url", "none");
		} else if (ptype.contains("移动")) {
			model.addAttribute("putong_js_yidong_url", "none");
		}
	}
	public Result showValidateImg(PhoneNum phoneNum, FormData fd) {
		Result r = new Result();
		if (isYiDong(phoneNum)) {
			JSYIDONG_Controller controller = (JSYIDONG_Controller)getBean(JSYIDONG_Controller.class);
			Map<String,Object> map = controller.putongFirst(fd.getRequest(),controller.getLogin(fd));
			 r.setForgetPassUrl(ConstantForgetPassword.jsyidong);
			 setResultOld(r, map);
			...
			JSDianXin_Controller controller = (JSDianXin_Controller) getBean(JSDianXin_Controller.class);
			//在map中需要有个url的key，为none是不需要验证码，否则返回验证码的链接
			Map<String,Object> map = controller.putongFirst(fd.getRequest(),controller.getLogin(fd));
