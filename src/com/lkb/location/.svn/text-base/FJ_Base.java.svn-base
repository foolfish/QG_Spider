...
/*福建*/
public class FJ_Base extends AbstractBase {
	
	private static Logger logger = Logger.getLogger(FJ_Base.class);
	public void goWhere(String ptype,String currentUser,Model model) {

		if (ptype.contains("移动")) {

			model.addAttribute("putong_fj_yidong_url", "none");
			...
			model.addAttribute("putong_fj_dianxin_url", "none");
		} else {
	...
	public Result showValidateImg(PhoneNum phoneNum, FormData fd) {
		...
		if (isYiDong(phoneNum)) {
			...
			FJYIDONG_Controller controller = (FJYIDONG_Controller) getBean(FJYIDONG_Controller.class);
			...
			Map<String,Object> map = controller.putongFirst(fd.getRequest(),controller.getLogin(fd));
			 r.setForgetPassUrl(ConstantForgetPassword.fjyidong);
			 setResultOld(r, map);
			...
			FJDianXin_Controller controller = (FJDianXin_Controller) getBean(FJDianXin_Controller.class);
			//在map中需要有个url的key，为none是不需要验证码，否则返回验证码的链接
			Map<String,Object> map = controller.putongFirst(fd.getRequest(),controller.getLogin(fd));
