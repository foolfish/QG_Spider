...
//-----------------------------------------
var encryptedString_st;
function init_1(string,keyPair) {
		var charSum = 0;
	...
	function encryption(encryptObject,keyPair) {
		...
		encryptedString_st = encrypted.substring(0, encrypted.length - 1);
		return encryptedString_st;
	}
	...
	//-----------keys对象
	//10001,长字符串,67
	function init(encryptionExponent, modulus, maxdigits) {
		var obj = new Object();
	...
	function getAll(p1,p2,p3,p4,p5) {
		var keys = init(p3, p4, p5) ;
		var name = init_1(p1,keys) ;
		var pass = init_1(p2,keys) ;
		return '{"phone":"'+name+'","pass":"'+pass+'"}';
	}