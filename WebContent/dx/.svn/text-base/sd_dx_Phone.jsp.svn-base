...
display:none;
					...
					<c:set var="currentNet" value="山东电信"/>
        			<c:set var="url" value="${putong_sd_dianxin_url}"/>
        			<c:set var="result" value="putong_sd_dianxin"/>
        			<form id="form1" class="base-form" action="">
						<h2 class="base-form-title">山东电信：</h2>
...
$(document).ready(function(){
	getInitAuth("/${ctx}/dx/sd/first.html");
});
function getAuth(){
	getInitAuth("/${ctx}/dx/sd/image.html");
}
...
function firstSubmit(){
	var vertifyUrl='/${ctx}/dx/sd/login.html';
   ...
   // ${result}("message").text("");
	...
	$.ajax({
	     ...
	     success : function(data) {
			   if(data.state){
			    //var spiderurl = "/${ctx}/dx/sd/startSpider.html";
...
function secondSubmit(){
	var mess = ${result}("message1");
	var vertifyUrl='/${ctx}/dx/sd/checkDynamics.html';
