...
display:none;
        			...
        			<c:set var="url" value="${putong_hlj_dianxin_url}"/>
        			<c:set var="result" value="putong_hlj_dianxin_url"/>
        			<form id="form1" class="base-form" action="">
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a class="base-forgetpwd" href="http://www.189.cn/dqmh/userCenter/optPwd/phoneOrTelResetPwd.jsp" target="_blank">忘记密码</a>
						</div>
					   <div style="padding-left: 120px">
...
function getAuth(){
	...
	$.ajax({
         ...
         async:true,
         data:{userName:'${phone}'},
	     url:'/${ctx}/putong_hlj_dianxin_GetAuth.html',
	     success : function(data) {
	    	 	...
	    	 	if(url != 'none'){		
	    	 		...
	    	 		//$("#authcode_img").attr("src", url + "?d=" + new Date().getTime());
...
function firstSubmit(){
	...
	issubmit = true;
	var vertifyUrl='/${ctx}/putong_hlj_dianxin_vertifyLogin.html';
...
function getSms(){
	...
	$.ajax({
	     type:'post',
       async:false,
	     url:'/${ctx}/putong_hlj_dianxin_GetSms.html',
...
function secondSubmit(){
	...
	issubmit = true;
	var vertifyUrl='/${ctx}/dongtai_hlj_dianxin_vertifyLogin.html';
