...
display:none;
					...
					<c:set var="currentNet" value="福建电信"/>
        			<c:set var="url" value="${putong_fj_dianxin_url}"/>
        			<c:set var="result" value="putong_fj_dianxin"/>
        			<form id="form1" class="base-form" action="">
						<h2 class="base-form-title">福建电信：</h2>
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a class="base-forgetpwd" href="http://fj.189.cn/service/pwdReset/pwdReset_SJ.jsp" target="_blank">忘记密码</a>
						</div>
						<div style="padding-left: 120px">
						...
						<div class="base-fieldset clearfix">
							...
							<img  onclick="getAuth()" src="<c:if test="${url ne 'none'}">${imgPath}${url}</c:if>" id="${result}authcode_img" style="height:30px;width:60px;position: relative;top: 8px;"/>
		  					...
		  					<label class="base-form-changelab">
		  						看不清？<a class="" href="javafjript:getAuth();">换一张</a>
		  					</label>
						</div>
...
$(document).ready(function(){
	getInitAuth("/${ctx}/dx/fj/first.html");
});
function getAuth(){
	getInitAuth("/${ctx}/dx/fj/image.html");
}
...
function firstSubmit(){
	var vertifyUrl='/${ctx}/dx/fj/login.html';
   ...
   // ${result}("message").text("");
	...
	$.ajax({
	     ...
	     success : function(data) {
			   if(data.state){
			   // var spiderurl = "/${ctx}/dx/fj/startSpider.html";
...
function secondSubmit(){
	var mess = ${result}("message1");
	var vertifyUrl='/${ctx}/dx/fj/checkDynamics.html';
