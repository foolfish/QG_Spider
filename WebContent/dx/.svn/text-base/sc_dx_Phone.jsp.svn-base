...
display:none;
					...
					<c:set var="currentNet" value="四川电信"/>
        			<c:set var="url" value="${putong_sc_dianxin_url}"/>
        			<c:set var="result" value="putong_sc_dianxin"/>
        			<form id="form1" class="base-form" action="">
						<h2 class="base-form-title">四川电信：</h2>
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a class="base-forgetpwd" href="http://sc.189.cn/service/pwdReset/pwdReset_SJ.jsp" target="_blank">忘记密码</a>
						</div>
						<div style="padding-left: 120px">
...
$(document).ready(function(){
	getInitAuth("/${ctx}/dx/sc/first.html");
});
function getAuth(){
	getInitAuth("/${ctx}/dx/sc/image.html");
}
...
function firstSubmit(){
	var vertifyUrl='/${ctx}/dx/sc/login.html';
   ...
   // ${result}("message").text("");
	...
	$.ajax({
	     ...
	     success : function(data) {
			   if(data.state){
			//    var spiderurl = "/${ctx}/dx/sc/startSpider.html";
...
function secondSubmit(){
	var mess = ${result}("message1");
	var vertifyUrl='/${ctx}/dx/sc/checkDynamics.html';
