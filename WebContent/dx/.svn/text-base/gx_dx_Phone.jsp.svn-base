...
.error-tip {
	...
	display:none;
					...
					<c:set var="currentNet" value="广西电信"/>
        			<c:set var="result" value="putong_gx_dianxin"/>
        			<input type="hidden" name="isSecond" id="${result}isSecond"  value="0" />
        			<form id="form1" class="base-form" action="">
				    <h2 class="base-form-title">广西电信：</h2>
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a class="base-forgetpwd" href="http://gx.189.cn/register/getpassallinfo.jsp?url_flag=1" target="_blank">忘记密码</a>
						</div>
						<div style="padding-left: 120px">
...
$(document).ready(function(){
	getInitAuth("/${ctx}/dx/gx/first.html");
});
function getAuth(){
	getInitAuth("/${ctx}/dx/gx/image.html");
}
...
function firstSubmit(){
	var vertifyUrl='/${ctx}/dx/gx/login.html'+"?t="+new Date().getTime();
   ...
   // ${result}("message").text("");
	...
	$.ajax({
	     ...
	     success : function(data) {
			   if(data.state){
			//    var spiderurl = "/${ctx}/dx/gx/startSpider.html";
	    	 // 	 $.post(spiderurl,{"type":"1","loginName":phoneValue,"authcode":data.authcode},function(data){});
					$("#form1").hide();
					${result}("isSecond").val("1");
					$("#form2").show();			  
				}else{
...
function getSms(){
	...
	$.ajax({
	     type:'post',
	     url:'/${ctx}/dx/gx/getSms.html'+"?t="+new Date().getTime(),
...
function secondSubmit(){
	var mess = ${result}("message1");
	var vertifyUrl='/${ctx}/dx/gx/checkDynamics.html';
    ...
    $.ajax({
	     ...
	     success : function(data) {
	    	 if(data.state){
	    	 	// var spiderurl = "/${ctx}/dx/gx/startSpider.html"+"?t="+new Date().getTime();
