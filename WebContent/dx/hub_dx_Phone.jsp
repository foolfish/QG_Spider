...
display:none;
        			...
        			<c:set var="url" value="${putong_hub_dianxin_url}"/>
        			<c:set var="result" value="putong_hub_dianxin"/>
        			<form id="form1" class="base-form" action="">
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a class="base-forgetpwd" href="http://hb.189.cn/pages/selfservice/custinfomanager/password/password.jsp?pagetype=resetpwd" target="_blank">忘记密码</a>
						</div>
						<div style="padding-left: 120px">
...
function getAuth(){
	...
	$.ajax({
	     ...
	     url:'/${ctx}/putong_hub_dianxin_GetAuth.html',
...
function checkField(){ 
	var flag = true;
	if(flag==true){
		$("#putong_hub_dianxin_message").text("正在努力登录中，请稍等……");
		$("#putong_hub_dianxin_message").show(500,firstSubmit);
	}
}

function firstSubmit(){
	var vertifyUrl='/${ctx}/putong_hub_dianxin_vertifyLogin.html';
	var phoneValue = $("#phone").val();
	     ...
	     success : function(data) {
			   ...
			   if(data.state==true){
				   window.location.href="/${ctx}/entranceEmail.html";
					...
					getAuth();
					 $("#putong_hub_dianxin_message").text(data.msg);
				}else{
					alert("登录失败,服务密码或验证码错误.");
					getAuth();
					 $("#putong_hub_dianxin_message").hide();
				}
	    	},
	    	 error:function(){
	    		parseFlag=false;
