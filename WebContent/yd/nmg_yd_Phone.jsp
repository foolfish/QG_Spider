...
display:none;
        			...
        			<c:set var="url" value="${putong_nmg_yidong_url}"/>
        			<c:set var="result" value="putong_nmg_yidong"/>
        			<form id="form1" class="base-form" action="">
        					<input type="hidden" name="resultCode" id="result_nmgyd_code" value="0"/>
							<input type="hidden" name="imgcode" id="imgcode_nmgyd" value=""/>
							
						    <input type="hidden" name="putong_nmg_yidong_userName1" id="putong_nmg_yidong_userName1" value=""/>
				...
				<c:if test="${url ne 'none'}">style="display:block;"</c:if>>
							...
							<img  onclick="checkAuth(1)" src="<c:if test="${url ne 'none'}">${imgPath}${url}</c:if>" id="putong_nmg_yidong_authcode_img" style="height:30px;width:60px;position: relative;top: 8px;"/>
					...
					<form id="form2" class="base-form base-element-hidden" action="">
						...
						<c:if test="${url ne 'none'}">style="display:block;"</c:if>>
							...
							<input class="base-fieldset-input w173" name="smsAuthCode" id="smsAuthCode" type="text"/>
		  					<label class="base-form-checkimglab">
							<img  onclick="smsAuthCode()" src="" id="sms_nmg_yidong_authcode_img" style="height:30px;width:60px;position: relative;top: 8px;"/>
...
function smsAuthCode(){
	...
	var phone = '${phone}';
		...
		$.ajax({
		     type:'post',
	         async:false,
		     url:'/${ctx}/putongnmgydSecond.html',
		     ...
		     success : function(data) {
		    	 ...
		    	 if(data.url!="none"){
					  ...
					  $('#sms_nmg_yidong_authcode_img').attr("src","${imgPath}"+url);
...
function changeAuth(){
	$("#putong_nmg_yidong_authcode_img").attr("src",$("#imgcode_nmgyd").val()+"&time="+new Date().getTime());	
}
	...
	function getydDynpass(){
		...
		if(flag == true){
			...
			$.ajax({
			     type:'post',
		         async:false,
			     url:'/${ctx}/nmg_yidong_sendCard.html',
	...
	function checkFwmm(type){
		if(type==1){
			var servicepw = $('#dongtai_nmg_yidong_password').val();
			if (servicepw.length != 6) {
				$("#dongtai_nmg_yidong_password_tip").text('请输入正确的6位动态密码！').show();		
				return false;
			} else if (isNaN(servicepw)) {
				$("#dongtai_nmg_yidong_password_tip").text('您输入的动态密码不正确！').show();		
				return false;
			}
			return true;
		}else if(type==2){
			var servicepw = $('#fuwu_nmg_yidong_password').val();
			if (servicepw.length != 6) {
				$("#fuwu_nmg_yidong_password_tip").text('请输入正确的6位服务密码！').show();		
				return false;
			} else if (isNaN(servicepw)) {
				$("#fuwu_nmg_yidong_password_tip").text('您输入的服务密码不正确！').show();		
				return false;
			}
			return true;
		}
	...
	function firstSubmit(){
		...
		var vertifyUrl='/${ctx}/fuwu_nmg_yidong_vertifyLogin.html';
	    ...
	    var style = $("#putong_nmg_yidong_authcode").attr("style");
		    	...
		    	// alert(data.flag)
		    	 if(data.flag=='true'){
			    	// alert(1);
				    	...
				    	checkAuth('1');
				    	$("#putong_nmg_yidong_authcode").show();
	...
	function secondSubmit(){
		var vertifyUrl='/${ctx}/dongtai_nmg_yidong_vertifyLogin.html';
