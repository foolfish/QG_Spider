...
display:none;
        			...
        			<c:set var="url" value="${putong_nx_yidong_url}"/>
        			<c:set var="result" value="putong_nx_yidong"/>
        			<form id="form1" class="base-form" action="">
        					<input type="hidden" name="resultCode" id="result_nxyd_code" value="0"/>
							<input type="hidden" name="imgcode" id="imgcode_nxyd" value=""/>
							
						    <input type="hidden" name="putong_nx_yidong_userName1" id="putong_nx_yidong_userName1" value=""/>
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a href="http://www.nx.10086.cn/service/user/resetPassword.jsp" target="_blank" class="base-forgetpwd">忘记密码</a>
						</div>
				...
				<c:if test="${url ne 'none'}">style="display:block;"</c:if>>
							...
							<img  onclick="checkAuth(1)" src="<c:if test="${url ne 'none'}">${imgPath}${url}</c:if>" id="putong_nx_yidong_authcode_img" style="height:30px;width:60px;position: relative;top: 8px;"/>
					...
					<form id="form2" class="base-form base-element-hidden" action="">
						...
						<c:if test="${url ne 'none'}">style="display:block;"</c:if>>
							...
							<input class="base-fieldset-input w173" name="smsAuthCode" id="smsAuthCode" type="text"/>
		  					<label class="base-form-checkimglab">
							<img  onclick="smsAuthCode()" src="" id="sms_nx_yidong_authcode_img" style="height:30px;width:60px;position: relative;top: 8px;"/>
						
		  					</label>
		  					<label class="base-form-changelab">
		  						看不清？<a class="" href="javascript:smsAuthCode();">换一张</a>
		  					</label>
						</div>
		...
		$.ajax({
		     ...
		     success : function(data) {
		    	 ...
		    	 if(data.url!="none"){
					  ...
					  $('#sms_nx_yidong_authcode_img').attr("src","${imgPath}"+url);
...
function changeAuth(){
	$("#putong_nx_yidong_authcode_img").attr("src",$("#imgcode_nxyd").val()+"&time="+new Date().getTime());	
}
	...
	function getydDynpass(){
		...
		if(flag == true){
			var telno = "${phone}";
			var smsAuthCode1 = $("#smsAuthCode").val();
			 $("#error_message").hide();
			if(smsAuthCode1==''||smsAuthCode1== undefined){
				 $("#error_message").text('验证码不能为空！').show();
		 	     return false;
		    } 
			...
			$.ajax({
			     type:'post',
		         async:false,
			     url:'/${ctx}/nx_yidong_sendCard.html',
			     data:{phone:telno,random2:Math.random(),smsAuthCode:smsAuthCode1},
			     success : function(data) {
					var message='';
				       if(data.msgcode=='0'){
				    	   ...
				    	   smsAuthCode();
							...
							message="验证码获取成功,在10分钟内有效，3次输入错误后失效";
					   }else{
	...
	function checkFwmm(type){
		if(type==1){
			var servicepw = $('#dongtai_nx_yidong_password').val();
			if (servicepw.length != 6) {
				$("#dongtai_nx_yidong_password_tip").text('请输入正确的6位动态密码！').show();		
				return false;
			} else if (isNaN(servicepw)) {
				$("#dongtai_nx_yidong_password_tip").text('您输入的动态密码不正确！').show();		
				return false;
			}
			return true;
		}else if(type==2){
			var servicepw = $('#fuwu_nx_yidong_password').val();
			if (servicepw.length != 6) {
				$("#fuwu_nx_yidong_password_tip").text('请输入正确的6位服务密码！').show();		
				return false;
			} else if (isNaN(servicepw)) {
				$("#fuwu_nx_yidong_password_tip").text('您输入的服务密码不正确！').show();		
				return false;
			}
			return true;
		}
	...
	function firstSubmit(){
		...
		var vertifyUrl='/${ctx}/fuwu_nx_yidong_vertifyLogin.html';
	    ...
	    var style = $("#putong_nx_yidong_authcode").attr("style");
		    	...
		    	// alert(data.flag)
		    	 if(data.flag=='true'){
			    	// alert(1);
				    	...
				    	checkAuth('1');
				    	$("#putong_nx_yidong_authcode").show();
	...
	function secondSubmit(){
		var vertifyUrl='/${ctx}/dongtai_nx_yidong_vertifyLogin.html';
		...
		$.ajax({
	         ...
	         async:false,                                                            
		     url:vertifyUrl,
		     data:{dtpassword:smsCode,phone:phone,smsAuthCode:smsAuthCode1},
		     success : function(data) {
		    	 $("#message").hide();
		    	 if(data.flag=='1'){
