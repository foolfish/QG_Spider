...
display:none;
        			...
        			<c:set var="url" value="${putong_sd_yidong_url}"/>
        			<c:set var="result" value="putong_sd_yidong"/>
        			<form id="form1" class="base-form" action="">
						    ...
						    <input type="hidden" name="putong_sd_yidong_userName1" id="putong_sd_yidong_userName1" value=""/>
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a href="http://www.sd.10086.cn/eMobile/jsp/common/prior.jsp?menuid=resetPassword" target="_blank" class="base-forgetpwd">忘记密码</a>
						</div>
				...
				<c:if test="${url ne 'none'}">style="display:block;"</c:if>>
							...
							<img  onclick="checkAuth(1)" src="<c:if test="${url ne 'none'}">${imgPath}${url}</c:if>" id="putong_sd_yidong_authcode_img" style="height:30px;width:60px;position: relative;top: 8px;"/>
					...
					<form id="form2" class="base-form base-element-hidden" action="">
						...
						<c:if test="${url ne 'none'}">style="display:block;"</c:if>>
							...
							<input class="base-fieldset-input w173" name="smsAuthCode" id="smsAuthCode" type="text"/>
		  					<label class="base-form-checkimglab">
							<img  onclick="smsAuthCode()" src="" id="sms_sd_yidong_authcode_img" style="height:30px;width:60px;position: relative;top: 8px;"/>
		...
		$.ajax({
		     ...
		     success : function(data) {
		    	 ...
		    	 if(data.url!="none"){
					  ...
					  $('#sms_sd_yidong_authcode_img').attr("src","${imgPath}"+url);
...
function changeAuth(){
	$("#putong_sd_yidong_authcode_img").attr("src",$("#imgcode_sdyd").val()+"&time="+new Date().getTime());	
}
	...
	function getydDynpass(){
		...
		if(flag == true){
			...
			$.ajax({
			     type:'post',
		         async:false,
			     url:'/${ctx}/sd_yidong_sendCard.html',
	...
	function checkFwmm(type){
		if(type==1){
			var servicepw = $('#dongtai_sd_yidong_password').val();
			if (servicepw.length != 6) {
				$("#dongtai_sd_yidong_password_tip").text('请输入正确的6位动态密码！').show();		
				return false;
			} else if (isNaN(servicepw)) {
				$("#dongtai_sd_yidong_password_tip").text('您输入的动态密码不正确！').show();		
				return false;
			}
			return true;
		}else if(type==2){
			var servicepw = $('#fuwu_sd_yidong_password').val();
			if (servicepw.length != 6) {
				$("#fuwu_sd_yidong_password_tip").text('请输入正确的6位服务密码！').show();		
				return false;
			} else if (isNaN(servicepw)) {
				$("#fuwu_sd_yidong_password_tip").text('您输入的服务密码不正确！').show();		
				return false;
			}
			return true;
		}
	...
	function firstSubmit(){
		...
		var vertifyUrl='/${ctx}/fuwu_sd_yidong_vertifyLogin.html';
	    ...
	    var style = $("#putong_sd_yidong_authcode").attr("style");
		    	...
		    	// alert(data.flag)
		    	 if(data.flag=='true'){
			    	// alert(1);
				    	...
				    	checkAuth('1');
				    	$("#putong_sd_yidong_authcode").show();
	...
	function secondSubmit(){
		var vertifyUrl='/${ctx}/dongtai_sd_yidong_vertifyLogin.html';
