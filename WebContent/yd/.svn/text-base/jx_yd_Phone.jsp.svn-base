...
display:none;
        			...
        			<c:set var="url" value="${putong_jx_yidong_url}"/>
        			<c:set var="result" value="putong_jx_yidong"/>
        			<form id="form1" class="base-form" action="">
						...
						<div class="base-fieldset login-fieldset clearfix">
							...
							<a class="base-forgetpwd" href="http://www.jx.10086.cn/my/user/toFwmmcz.do" target="_blank">忘记密码</a>
						</div>
						<div style="padding-left: 120px">
				    ...
				    <div style="padding-left: 120px">
						...
						<div class="base-fieldset clearfix">
							...
							<input class="base-fieldset-input w173" name="authCode2" id="${result}authCode2" type="text"/>
		  					<label class="base-form-checkimglab">
							<img  onclick="getAuth()" src="<c:if test="${url ne 'none'}">${imgPath}${url}</c:if>" id="${result}authcode_img2" style="height:30px;width:60px;position: relative;top: 8px;"/>
...
$(document).ready(function(){
	getInitAuth("/${ctx}/yd/jx/first.html");
});
function getAuth(){
	getInitAuth("/${ctx}/yd/jx/image.html");
}
...
function firstSubmit(){
	var vertifyUrl='/${ctx}/yd/jx/login.html';
...
function secondSubmit(){
	var mess = ${result}("message1");
	var vertifyUrl='/${ctx}/yd/jx/checkDynamics.html';
